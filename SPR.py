""" Multiple / Single Sources shortest distance  """
...
class attacker :
    ...
    def __str__(self):
        return "Attacker at " + str(self.location) + " and visited " + str(len(self.visitedLocations)) +\
               " nodes till now."
    def __init__(self):
...
def engine(final_list,final,radio_range,no_of_packets,selectNodes,isTestBattery):
    ...
    def hop_control(ord):
        ...
        while 1:
            ...
            transferEnergy = sendEnergy + receiveEnergy  # total energy utilised

            temp_remEnergy = nodes[index].energy - transferEnergy  # updating the remaining energy
...
def simulation_control(no_of_simulations,length,selectNodes,csv_path,isCreate,isUpload,scenario,areaName,isTestBattery):
    ...
    for simulation in tqdm(range(no_of_simulations),desc=barName,ascii=False,ncols=100,leave=False):
        ...
        hopCount,distCount,result,safety_period,energy_consumed,avgHops,entropy,sentPackets = engine(final_list,final,radio_range,no_of_packets,selectNodes,isTestBattery)
    ...
    # csv_path = '/Users/apple/Class/Observations/Data_Collected/high_short_multiple_sources_e.csv'
    if isUpload:
...
def input_control():
    ...
    for a in tqdm(range(count),desc="Process : ",ascii=False,ncols=100,mininterval=0.1):
        ...
        if isSingle:
            path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Shortest_Path/fshort_single_sources.csv'
            scenario = "S"

            simulation_control(no_of_simulations, baseLenghts, singlebaseSNlOcs[a], path, 1, isUpload, scenario, area,isTestBattery)

            for i, length in tqdm(enumerate(lengths), desc="Process : ", ascii=False, ncols=100, miniters=1,leave=False):
                simulation_control(no_of_simulations, length, singleSourceNodeLocs[i], path, 0, isUpload, scenario,
                                   areas[i],isTestBattery)
        else:

            if a == 0:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Shortest_Path/fshort_multiple_sources_b.csv'
            elif a == 1:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Shortest_Path/fshort_multiple_sources_c.csv'
            elif a == 2:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Shortest_Path/fshort_multiple_sources_d.csv'
            else:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Shortest_Path/fshort_multiple_sources_e.csv'
