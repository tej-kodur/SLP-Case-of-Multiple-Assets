""" Phantom Route using Pure Random Path and L-shaped path """
...
def engine(side, final_list, final, radio_range, offset_dist, randhops, no_of_packets, selectNodes,isTestBattery):
    attacker1 = attacker()
    side_nodes = round(side / offset_dist, 0)
    ...
    def lPath(cord, neighbour_list, isVertical, isUporRight):

        if isVertical:
            if isUporRight:
                req_ord = cord[1] + offset_dist
            else:
                req_ord = cord[1] - offset_dist
            ...
            if isUporRight:
                req_ord = cord[0] + offset_dist
            else:
                req_ord = cord[0] - offset_dist
            ans = [req_ord, cord[1]]

        # print("lpath\ncord:",cord,"isvertical : ",isVertical,"isUp or right",isUporRight,"req ord",req_ord,"ans:",ans,"\n")
        if ans in neighbour_list:
            return ans
        else:
            return 0

    def getRandNum(count):
        req_num = random.randint(8, side_nodes)
        req_count = count + req_num
        return req_count
    ...
    def hop_control(ord):
        ...
        vertical_count = 0
        horizontal_count = 0
        pathSelect = lambda x: 1 if x != 0 else 0
        directSelect = random.randint(0, 1)
        ...
        while 1:
            ...
            if [0, 0] in temp_neigh:
                if count < randhops:
                ...
                elif count < vertical_count or count < horizontal_count:

                    ans = lPath(cord, temp_neigh, pathSelect(vertical_count), directSelect)
                    ...
                    if ans == 0:
                        if vertical_count == 0:
                            ...
                            horizontal_count = getRandNum(count)
                            vertical_count = 0
                            directSelect = random.randint(0, 1)
                            ans = lPath(cord, temp_neigh, pathSelect(vertical_count), directSelect)
            ...
            if count == randhops - 1:
                vertical_count = getRandNum(count)

            if count == vertical_count - 1:
                horizontal_count = getRandNum(count)
                directSelect = random.randint(0, 1)
                vertical_count = 0
...
def simulation_control(no_of_simulations, length, selectNodes, csv_path, isCreate, isUpload,scenario,areaName,isTestBattery):
    ...
    for simulation in tqdm(range(no_of_simulations), desc=barName, ascii=False, ncols=100,leave=False):
        ...
        hopCount, distCount, result, safety_period, energy_consumed, avgHops, entropy,sentPackets = engine(length, final_list,
                                                                                               final, radio_range,
                                                                                               offsetDist, randhops,
                                                                                               no_of_packets,
                                                                                               selectNodes,isTestBattery)
    ...
    # csv_path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Lpath_pureRandom/flPath_pRandom_multiple_sources_e.csv'
    if isUpload:
...
def input_control():
    ...
    print("\nL-Path Pure Random Walk Simulation\nPROCESS STARTED\n")

    for a in tqdm(range(count), desc="Process : ", ascii=False, ncols=100, miniters=1):

        if isSingle:
            path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Lpath_pureRandom/flPath_pRandom_single_source.csv'
            scenario = "S"
            ...
            if a == 0:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Lpath_pureRandom/flPath_pRandom_multiple_sources_b.csv'
            elif a == 1:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Lpath_pureRandom/flPath_pRandom_multiple_sources_c.csv'
            elif a == 2:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Lpath_pureRandom/flPath_pRandom_multiple_sources_d.csv'
            else:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Lpath_pureRandom/flPath_pRandom_multiple_sources_e.csv'
