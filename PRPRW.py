...
class attacker :
    ...
    def __str__(self):
        return "Attacker at " + str(self.location) + " and visited " + str(len(self.visitedLocations)) + " nodes till now."
    def __init__(self):
...
class node:
    ...
    def __str__(self):
        return  str(self.type)+ " Node Id " + str(self.id) + " at " + str(self.location) + " with energy " + str(self.energy) + " and status is " + str(self.status) + " also sent " + str(self.sentpackets) + " packets till now."
...
def engine(final_list,final,radio_range,randhops,no_of_packets,selectNodes,isTestBattery):

    attacker1 = attacker()
    no_of_SourceNodes = len(selectNodes)

    def index_to_locations(selectNodes):
        source_node_pos = []
    ...
    def hop_control(ord):
        ...
        while 1:
            ...
            if [0, 0] in temp_neigh:
                if count < randhops:
                    req_ord = random.sample(temp_neigh, 1)
                    ans = req_ord[0]
                else:
                    ans = [0, 0]
                ...
                if count < randhops:
                    ...
                    min_ans = min(temp_dist)
                    index_ans = temp_dist.index(min_ans)
                    ans = temp_neigh[index_ans]  # ------> selected node
    ...
    def display_details():
        ...
        for i in range(len(nodes)):
            ...
            nodes[i].id, ":", nodes[i].location, ":", nodes[i].initialEnergy, ":", nodes[i].energy, ":", nodes[i].status,
...
def simulation_control(no_of_simulations,length,selectNodes,csv_path='',isCreate=0,isUpload=0,scenario=0,areaName=0,isTestBattery=False):
    # ---Required Details--- #
    # length = 1000
    breadth = length
    radio_range = 71
    randhops = 10
    no_of_packets = 10000 #400  # per each souce
    ...
    type = ["TYPE","Simulations","Length","Breadth","Area","Radio range","Offset Distance","packets sending","Randomhops"]
    ...
    for simulation in tqdm(range(no_of_simulations),desc=barName,ascii=False,ncols=100,leave=False):
        ...
        hopCount, distCount, result, safety_period, energy_consumed, avgHops, entropy,sentPackets = engine(final_list,final,radio_range,randhops,no_of_packets,selectNodes,isTestBattery)

        sumAvgHops += avgHops
        sumEntropy += entropy
        sumSentPackets += sentPackets

        data2t = ["Hops","Distance","Safety Period","Energy Consumed","Average Hops","Entropy","Packets Sent","Result"]
        data2d = [hopCount,distCount,safety_period,energy_consumed,avgHops,entropy,sentPackets,result]
        type.extend(data2t)
        data.extend(data2d)
    ...
    data3t = ["Average Packets Sent","Average Entropy","Average of Average Hops"," Average Safety Period", "Average Energy Consumed (nJ)","Average Energy Consumed (J)","Times Attacker Succeeded","Capture Ratio","Capture Percentage"]
    data3d = [avgSentPackets,avgEntropy,avgAvgHops,safety_period_avg,energy_consumed_avg,energy_consumed_avg_J,attacker_succeed_count,capture_ratio,capture_percentage]
    type.extend(data3t)
    data.extend(data3d)
    ...
    # csv_path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Pure_Random/fpure_multiple_sources_e.csv'
    if isUpload:
...
def input_control():
    ...
    sourceNodeLocs = [[[116,102],[149,133],[227,207],[272,250],[321,297],[431,403]],     # b
                     ...
                     [[118,174,223],[151,223,287],[229,339,439],[274,527,406],[323,623,479],[433,839,643]],
                     [[118,174,223,62],[151,223,287,79],[229,339,439,119],[274,527,406,142],[323,623,479,167],[433,839,643,223]]]
    ...
    for a in tqdm(range(count),desc="Process : ",ascii=False,ncols=100,mininterval=0.1):

        if isSingle:
            path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Pure_Random/fpure_single_source.csv'
        else:
            if a == 0:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Pure_Random/fpure_multiple_sources_b.csv'
            elif a == 1:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Pure_Random/fpure_multiple_sources_c.csv'
            elif a == 2:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Pure_Random/fpure_multiple_sources_d.csv'
            else:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Pure_Random/fpure_multiple_sources_e.csv'
        ...
        # print("Generating Scenario : ",scenario)
        # print("Minimum Area Generating")
        # print("File Created")
        if isSingle:
        ...
        for i , length in enumerate(lengths):
            # print("Existing File\nIncreasing Area")
            if isSingle:
                simulation_control(no_of_simulations,length,singleSourceNodeLocs[i],path,0,isUpload,scenario,areas[i],isTestBattery)
            else:
                simulation_control(no_of_simulations, length, sourceNodeLocs[a][i], path, 0, isUpload, scenario,areas[i],isTestBattery)
            # print("Simulations Ended : ")
            # print("Data uploaded to file successfully\n")

        # print("Scenario",  "Ended Successfully\n")
