""" Square topology with multiple sources/single source and Biased Coin Random Walk without shortest path
(Used mostly for Forward Walk) """
...
def engine(final_list,final,radio_range,offset_dist,no_of_packets,selectNodes,isForward=1,isTestBattery=False):

    attacker1 = attacker()

    no_of_SourceNodes = len(selectNodes)

    def index_to_locations(selectNodes): #For getting locations of the given source node IDs
        source_node_pos = []
...
def simulation_control(no_of_simulations,length,selectNodes,csv_path,isCreate,isUpload,scenario,areaName,isTestBattery):
    ...
    data = ["DATA", no_of_simulations, length, breadth, area, radio_range,offsetDist ,no_of_packets]
    ...
    for simulation in tqdm(range(no_of_simulations),desc=barName,ascii=False,ncols=100,leave=False):
        ...
        hopCount,distCount,result,safety_period,energy_consumed,avgHops,entropy,sentPackets = engine(final_list,final,radio_range,offsetDist,no_of_packets,selectNodes,isForward,isTestBattery)
    ...
    # csv_path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Forward_Walk/fForward_multiple_sources_e.csv'
    if isUpload:
    ...
    # print(""\nSafety Period average : {}".format(safety_period_avg))
    # print("\nEnergy Consumed average : {}".format(energy_consumed_avg))
...
def input_control():
    ...
    print("\nForward Walk Simulation (Without Shortest Path)\nPROCESS STARTED\n")
    for a in tqdm(range(count),desc="Process : ",ascii=False,ncols=100):

        if isSingle:
            path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Forward_Walk/fForward_single_source.csv'
        else:
            if a == 0:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Forward_Walk/fForward_multiple_sources_b.csv'
            elif a == 1:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Forward_Walk/fForward_multiple_sources_c.csv'
            elif a == 2:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Forward_Walk/fForward_multiple_sources_d.csv'
            else:
                path = '/Users/apple/Project_1/Observations/Data_Collected/New_Data/Forward_Walk/fForward_multiple_sources_e.csv'
        ...
        for i , length in enumerate(lengths):
            ...
            if isSingle:
                ...
                simulation_control(no_of_simulations, length, sourceNodeLocs[a][i], path, 0, isUpload, scenario,
                                   areas[i],isTestBattery)
            # print("Simulations Ended : ")
            # print("Data uploaded to file successfully\n")
        #
